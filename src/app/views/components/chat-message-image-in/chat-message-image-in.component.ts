...
@Component({
  selector: 'app-chat-message-image-in',
  standalone: true,
  imports: [MarkdownModule],
  templateUrl: './chat-message-image-in.component.html',
  styles: ``,
})
export class ChatMessageImageInComponent implements AfterViewInit {
  @Input({ required: true }) message!: string;
  @Input({ required: true }) url!: string;

  @Output()
  private onEmitImage = new EventEmitter<string>();

  @ViewChild('imageCanvas')
  private canvasElement?: ElementRef<HTMLCanvasElement>;

  private originalImage = signal<HTMLImageElement | null>(null);
  ...
  onMouseUp() {
    ...
    this.onEmitImage.emit(url);
  }

  handleEmit() {
    this.onEmitImage.emit(this.url);
  }
}
